      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-[#8BC5E326]">
            <TableHead className="text-[#464255] font-medium">
              Services
            </TableHead>
        ...
        <TableBody>
          {paginatedPayments.map((payment, index) => (
            <TableRow key={index} className="border-b-2 border-[#C0DBFB5E]">
              <TableCell className="py-4 text-[#464255]">
                {payment.service}
              </TableCell>
              <TableCell className="py-4 text-[#464255]">
                {payment.amount}
              ...
              <TableCell className="py-4">
                <span
                  className={`px-3 py-1 rounded-full text-sm ${
                    payment.status === "Paid"
                      ? "bg-[#f2f5f3] text-[#81d17c]"
                      : "bg-[#fff8f3] text-[#fb5b01]"
                  }`}
                >
                  {payment.status}
                </span>
              ...
              <TableCell className="py-4">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onClick={() =>
                        alert(`View details for ${payment.service}`)
                      }
                    >
                      View Details
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => alert(`Edit ${payment.service}`)}
                    >
                      Edit
                    </DropdownMenuItem>
                    {payment.status === "Pending" && (
                      <DropdownMenuItem onClick={() => markAsPaid(index)}>
                        Mark as Paid
                      </DropdownMenuItem>
                    )}
                    <DropdownMenuSeparator />
